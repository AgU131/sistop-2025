...
cat weather_cordoba.in | sort -k5 -n -r | head -1 | awk '{print $1"-"$2"-"$3}'

# EJERCICIO 4B: Día de mínima temperatura en Córdoba
cat weather_cordoba.in | sort -k6 -n | head -1 | awk '{print $1"-"$2"-"$3}'

# EJERCICIO 5: Ordenar jugadores de tenis por ranking
cat atpplayers.in | sort -k3 -n 

# EJERCICIO 6: Ordenar tabla de Superliga por puntos y diferencia de goles
awk '{print $0, ($7-$8)}' superliga.in | sort -k2,2nr -k9,9nr | cut -d' ' -f1-8
